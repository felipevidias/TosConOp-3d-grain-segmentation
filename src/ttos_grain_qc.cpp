...
{
    if (z < 0 || z >= static_cast<int>(qc.size())) return;
    ...
    std::vector<cv::Mat>& out_rgb)
{
    ...
    for (const FinalGrain& g : grains) {
        ...
        for (int z = 0; z < depth; ++z) {
            ...
            const int cx = std::max(0, std::min(width - 1, static_cast<int>(std::round(static_cast<double>(sx[z]) / count[z]))));
            const int cy = std::max(0, std::min(height - 1, static_cast<int>(std::round(static_cast<double>(sy[z]) / count[z]))));

            cv::circle(out_rgb[z], cv::Point(cx, cy), 1, cv::Scalar(color[0], color[1], color[2]), -1, cv::LINE_AA);
    ...
    for (int z = 0; z < depth; ++z) {
        for (int y = 0; y < height; ++y) {
            ...
            for (int x = 0; x < width; ++x) {
                if (srcp[x] > dstp[x]) dstp[x] = srcp[x];
            }
        }
    }
...
static cv::Mat make_mip_xz(const std::vector<cv::Mat>& volume)
{
    ...
    for (int z = 0; z < depth; ++z) {
        ...
        for (int y = 0; y < height; ++y) {
            const uchar* srcp = volume[z].ptr<uchar>(y);
            for (int x = 0; x < width; ++x) {
                if (srcp[x] > dst_row[x]) dst_row[x] = srcp[x];
            }
        }
    }
...
static cv::Mat make_mip_yz(const std::vector<cv::Mat>& volume)
{
    ...
    for (int z = 0; z < depth; ++z) {
        ...
        for (int y = 0; y < height; ++y) {
            ...
            uchar vmax = 0;
            for (int x = 0; x < width; ++x) {
                if (srcp[x] > vmax) vmax = srcp[x];
            }
            if (vmax > dst_row[y]) dst_row[y] = vmax;
        }
    }
...
static void draw_seed_dot(cv::Mat& img, int u, int v, const cv::Vec3b& color)
{
    ...
    for (int dv = -radius; dv <= radius; ++dv) {
        for (int du = -radius; du <= radius; ++du) {
            ...
            if (uu < 0 || uu >= img.cols || vv < 0 || vv >= img.rows) continue;
...
cv::Mat render_track_orthoview_qc(
    ...
    const std::vector<FinalGrain>& grains)
{
    return render_seed_orthoview_qc(volume, grains);
}
